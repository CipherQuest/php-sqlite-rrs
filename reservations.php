...
?>
<style>
    #reservationTBL .btn-group .btn-sm{
        line-height: .9rem !important;
        padding: 5px;
    }
    #reservationTBL .btn-group .material-symbols-outlined{
        line-height: .9rem !important;
...
<h1 class="text-center fw-bolder">List of Reservations</h1>
            ...
            <div class="container-fluid">
                <div class="mb-3">
                    <div class="row align-items-end justify-content-center">
                        ...
                        <div class="col-lg-auto">
                            <a class="btn btn-light border btn-sm rounded-0 d-flex align-items-center" href="./?page=manage_reservation"><span class="material-symbols-outlined">add</span> Add Reservation</a>
                ...
                <div class="table-responsive">
                    <table class="table table-sm table-bordered table-hover table-striped" id="reservationTBL">
                        <colgroup>
                            <col width="5%">
                            <col width="20%">
                            <col width="20%">
                        ...
                        <thead>
                            <tr>
                                ...
                                <th class="text-center">Cottage/Room</th>
                        ...
                        <tbody>
                            ...
                            // $to = $to->format("Y-m-d");
                            $i = 1;
                            $reservations_sql = "SELECT *, (SELECT `room_no` FROM `room_list` where `room_list`.`room_id` = `reservation_list`. `room_id`) as room FROM `reservation_list` where ('{$from}' BETWEEN `from_date` and `to_date`) OR ('{$to}' BETWEEN `from_date` and `to_date`) OR (`from_date` BETWEEN '{$from}' and '{$to}') OR (`to_date` BETWEEN '{$from}' and '{$to}')  ORDER BY strftime('%s', `date_created`) desc";
                            
                            $reservations_qry = $conn->query($reservations_sql);
                            while($row = $reservations_qry->fetchArray()):
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <div class="btn-group btn-group-sm">
                                        <a class="btn btn-sm btn-outline-dark rounded-0 view_data" href="./?page=view_reservation&id=<?= $row['reservation_id'] ?>" data-id='<?= $row['reservation_id'] ?>' title="View reservation"><span class="material-symbols-outlined">subject</span></a>
                                        <a class="btn btn-sm btn-outline-primary rounded-0 edit_data" href="./?page=manage_reservation&id=<?= $row['reservation_id'] ?>" data-id='<?= $row['reservation_id'] ?>' title="Edit reservation"><span class="material-symbols-outlined">edit</span></a>
                                        <button class="btn btn-sm btn-outline-danger rounded-0 delete_data" type="button" data-id='<?= $row['reservation_id'] ?>' title="Delete reservation"><span class="material-symbols-outlined">delete</span></button>
                                    </div>
                                </td>
                            </tr>
                            <?php endwhile; ?>
                            <?php if(!$reservations_qry->fetchArray()): ?>
                                <tr>
...
<script>
    $(function(){
        ...
        $('.notify_data').on('click', function(e){
            ...
            var id = $(this).attr('data-id');
            start_loader()
            var _conf = confirm(`Are you sure to notify again the queued reservation?`);
            if(_conf === true){
                $.ajax({
        ...
        $('#filter').click(function(e){
            ...
            var to = $('#date_to').val()
            location.replace(`./?page=reservations&from=${from}&to=${to}`)
        })

        $('#search').on('input change', function(e){
            var _search = $(this).val().toLowerCase();
            $('#reservationTBL tbody tr').each(function(){
