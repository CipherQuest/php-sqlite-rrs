...
if(isset($_GET['id']) && $_GET['id'] > 0){
    ...
    $sql = "SELECT * FROM `fee_list` where `fee_id` = '{$_GET['id']}' ";
    $query = $conn->query($sql);
    $data = $query->fetchArray();

}

// Generate Manage fee Form Token
$_SESSION['formToken']['fee-form'] = password_hash(uniqid(),PASSWORD_DEFAULT);
?>
<h1 class="text-center fw-bolder"><?= isset($data['fee_id']) ? "Update Fee Details" : "Add New Fee" ?></h1>
    ...
    <div class="card rounded-0">
        <div class="card-body">
            <div class="container-fluid">
                <form action="" id="fee-form">
                    <input type="hidden" name="formToken" value="<?= $_SESSION['formToken']['fee-form'] ?>">
                    <input type="hidden" name="fee_id" value="<?= $data['fee_id'] ?? '' ?>">
                    <div class="mb-3">
                        <label for="name" class="text-body-tertiary">Name</label>
                        <input type="text" class="form-control rounded-0" id="name" name="name" required="required" autofocus  value="<?= $data['name'] ?? "" ?>">
                    </div>
                    <div class="mb-3">
        ...
        <div class="card-footer">
            <div class="row justify-content-evenly">
                <button class="btn col-lg-4 col-md-5 col-sm-12 col-12 btn-primary rounded-0" form="fee-form">Save</button>
                <a class="btn col-lg-4 col-md-5 col-sm-12 col-12 btn-secondary rounded-0" href='./?page=fees'>Cancel</a>
            </div>
        </div>
    </div>
</div>
                ...
                success:function(resp){
                    if(resp.status == 'success'){
                        if('<?= $_GET['toview'] ?? "" ?>' == ""){
                            ...
                            location.replace("./?page=view_fee&id=<?= $data['fee_id'] ?? "" ?>");
                        }
                    }else{
                        _el.addClass('alert alert-danger')
                    }
