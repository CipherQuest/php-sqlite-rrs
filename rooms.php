...
?>
<style>
    #roomTBL .btn-group .btn-sm{
        line-height: .9rem !important;
        padding: 5px;
    }
    #roomTBL .btn-group .material-symbols-outlined{
        line-height: .9rem !important;
...
<h1 class="text-center fw-bolder">List of Rooms</h1>
            ...
            <div class="container-fluid">
                <div class="mb-3">
                    <div class="row align-items-end justify-content-center">
                        ...
                        <div class="col-lg-auto">
                            <a class="btn btn-light border btn-sm rounded-0 d-flex align-items-center" href="./?page=manage_room"><span class="material-symbols-outlined">add</span> Add Room</a>
                ...
                <div class="table-responsive">
                    <table class="table table-sm table-bordered table-hover table-striped" id="roomTBL">
                        <colgroup>
                            <col width="5%">
                            <col width="20%">
                            <col width="20%">
                        ...
                        <tbody>
                            ...
                            $to = $to->format("Y-m-d");
                            $i = 1;
                            $rooms_sql = "SELECT * FROM `room_list` where date(`date_created`) BETWEEN '{$from}' and '{$to}' ORDER BY strftime('%s', `date_created`) desc";
                            
                            $rooms_qry = $conn->query($rooms_sql);
                            while($row = $rooms_qry->fetchArray()):
                            ...
                            <tr>
                                ...
                                <td class="">
                                    <div class="lh-1">
                                        <div class="text-secondary">Room #: <?= $row['room_no'] ?></div>
                                        <div><?= $row['name'] ?></div>
                                    </div>
                                </td>
                                ...
                                <td class="text-center">
                                    <div class="btn-group btn-group-sm">
                                        <a class="btn btn-sm btn-outline-dark rounded-0 view_data" href="./?page=view_room&id=<?= $row['room_id'] ?>" data-id='<?= $row['room_id'] ?>' title="View room"><span class="material-symbols-outlined">subject</span></a>
                                        <a class="btn btn-sm btn-outline-primary rounded-0 edit_data" href="./?page=manage_room&id=<?= $row['room_id'] ?>" data-id='<?= $row['room_id'] ?>' title="Edit room"><span class="material-symbols-outlined">edit</span></a>
                                        <button class="btn btn-sm btn-outline-danger rounded-0 delete_data" type="button" data-id='<?= $row['room_id'] ?>' title="Delete room"><span class="material-symbols-outlined">delete</span></button>
                                    </div>
                                </td>
                            </tr>
                            <?php endwhile; ?>
                            <?php if(!$rooms_qry->fetchArray()): ?>
                                <tr>
...
<script>
    $(function(){
        ...
        $('.notify_data').on('click', function(e){
            ...
            var id = $(this).attr('data-id');
            start_loader()
            var _conf = confirm(`Are you sure to notify again the queued room?`);
            if(_conf === true){
                $.ajax({
        ...
        $('#filter').click(function(e){
            ...
            var to = $('#date_to').val()
            location.replace(`./?page=rooms&from=${from}&to=${to}`)
        })

        $('#search').on('input change', function(e){
            var _search = $(this).val().toLowerCase();
            $('#roomTBL tbody tr').each(function(){
